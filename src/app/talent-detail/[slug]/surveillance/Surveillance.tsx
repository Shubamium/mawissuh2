...
export default function Surveillance({ videoId }: Props) {
      ...
      <div className={`surv-body ${isCasted ? "casted" : "preparing"}`}>
        <div className="spell-part">
          <div className="spell rune-text">
            {isCasted &&
              spells.map((spellLine, index) => {
                return (
                  <p key={"spell-line" + (index + 1)} className="spell-line">
                    {spellLine.split("").map((word, charIndex) => {
                      const delay = index * 1000 + charIndex * 50;

                      return (
                        <span
                          key={"spell-word" + charIndex}
          ...
          >
            CAST
            <img
              src="/decors/small_rune.png"
              alt=""
              className="decor_btn-rune"
            />
          </button>
        </div>
        {videoId === undefined || videoId === "" ? (
          <div className="no-reach">
            <p className="rune-text">OKMEDO-DKENDIB-DKWDJOD</p>
            <p className="warn">- OUT OF REACH - </p>
          </div>
        ) : (
          <iframe
            className="vid-frame"
